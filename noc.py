...
try:
    with open('C:\\Users\\HP\\OneDrive\\Desktop\\NoC_Router-main (2)\\NoC_Router-main\\Log_File.txt', 'w') as output_file:
        ...
        class Router:
            ...
            def __str__(self):
                ...
                if self.input_buffer is not None:
                    s += f"Input Buffer Value: {self.input_buffer[2]}, Delay: {buffer_delay}\n"
                else:
                    s += "Input Buffer Value: None\n"

                if self.switch_allocator is not None:
                    s += f"Switch Allocator Value: {self.switch_allocator[2]}, Delay: {sa_delay}\n"
                else:
                    s += "Switch Allocator Value: None\n"

                if self.crossbar is not None:
                    s += f"CrossBar Value: {self.crossbar[2]}, Delay: {xbar_delay}\n"
                else:
                    s += "CrossBar Value: None\n"
        ...
        if __name__ == '__main__':
            ...
            try:
                flagSarva = 0
                #conv.run()
                with open('C:\\Users\\HP\\OneDrive\\Desktop\\NoC_Router-main (2)\\NoC_Router-main\\traffic.txt', 'r') as file:
            ...
            try:
                with open('C:\\Users\\HP\\OneDrive\\Desktop\\NoC_Router-main (2)\\NoC_Router-main\\delays.txt', 'r') as file1:
            ...
            num_routers = 9
            buffer_delays = generate_gaussian_delays(mean_delays[0], mean_delays[0]*0.1, num_routers)
            sa_delays = generate_gaussian_delays(mean_delays[1], mean_delays[1]*0.1, num_routers)
            xbar_delays = generate_gaussian_delays(mean_delays[2], mean_delays[2]*0.1, num_routers)

            all_routers = {i: Router(i) for i in range(0, 9)}

            for i in range(num_routers):
                all_routers[i].buffer_delay = buffer_delays[i]
                all_routers[i].sa_delay = sa_delays[i]
                all_routers[i].xbar_delay = xbar_delays[i]


            buffer_delay = all_routers[0].buffer_delay
            sa_delay = all_routers[0].sa_delay
            xbar_delay = all_routers[0].xbar_delay
            ...
            while((not all_empty(all_routers)) or len(traffic_file) != 0):
                ...
                while j <= 8:
                    try:
                        ...
                        with open('C:\\Users\\HP\\OneDrive\\Desktop\\NoC_Router-main (2)\\NoC_Router-main\\Log_File.txt', 'a') as log_file:
                            log_file.write(f"At clock cycle: {clock} = {all_routers[j]}\n")
                    except Exception as e:
...
print("Buffer Delays:", buffer_delays)
print("\n")
print("Switch Allocator Delays:", sa_delays)
print("\n")
print("Crossbar Delays:", xbar_delays)
